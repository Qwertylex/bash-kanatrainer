#!/bin/bash
#Alex's Kana trainer alpha0.92 (fork of my [internal] pre-alpha Hiratrainer)
...
while [ `echo $choice | tr [:upper:] [:lower:]` == hira ]; do
hira=($hiras)
num_hiras=${#hira[*]}
...
while [ `echo $choice | tr [:upper:] [:lower:]` == kata ]; do
kata=($katas)
num_katas=${#kata[*]}
